...
function Expense() {
  ...
  const getExpenses = async () => {
    try {
      ...
      setExpenseVal(obj.total_expense);
      setExpenseList(obj.expense_items);
    } catch (err) {
      setTableDisplayErr(
  ...
  return (
    <div>
      <h4 className="mx-3 mt-4">Total Expense: ₹{expenseVal}</h4>
        ...
        <div className="card card-body bg-info-subtle">
          {expenseList && expenseList.length > 0 ? (
            ...
            <p className="text-success">
              No expense items here! Add one below.
            </p>
          )}
        </div>
      </div>
      ...
      <form onSubmit={sendExpense}>
        <div className="row g-3 align-items-center m-3">
          ...
          <span id="nameinfo" className="form-text mx-1">
            Pick a name for this expense item (like "Car" or "Groceries")
          </span>
        </div>
